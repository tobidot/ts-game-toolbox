...
export class Slider extends Element {
    ...
    public override drag(start_coords: Vector2 | null, current_coords: Vector2I): boolean {
    ...
    public set_value(x: number) {
        ...
        const new_value = this.min + (this.max - this.min) * Math.max(0, Math.min(1, relative_x));
    ...
    public override draw(ctx: CanvasRenderingContext2D): void {
        ...
        const thumb_x = this.rect.left + progress * this.rect.width - thumb_width / 2;
        ...
        ctx.fillText(`${this.title}: ${this.value.toFixed(2)}`, this.rect.left, track_y - 5);
    }
