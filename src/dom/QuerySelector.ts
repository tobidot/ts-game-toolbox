 ...
 * @param selector
 */
export function get_element_by_query_selector(
...
export function get_element_by_query_selector<T extends HTMLElement>(
    ...
    if (!is_of_class_or_html_element<T>(element, class_type)) {
        throw new Error(
            "Element not of required type HTML Element " +
                (class_type?.name ?? "HtmlElement"),
        );
    }
    return element;
}
 ...
 * @param selector
 */
export function get_element_by_class_name(
 ...
 * @returns
 */
function is_of_class_or_html_element<T extends Element>(
    element: Element,
    class_type?: Class<T>,
): element is T {
    if (class_type) {
